...
function wppp_render_paypal_button_with_other_amt($args) {
    ...
    $output .= '<div class="wp_pp_button_amount_section">'.esc_attr($other_amount_label).' <br/><input type="number" min="1" step="any" name="amount" value="' . esc_attr($default_amount) . '" class="pqp_other_amt_input" required><br/>' . esc_attr($currency) . '</div>';
...
<option value="HKD">HKD - Hong Kong Dollar </option>
    ...
    if (!empty($validate_ipn)) {
        $notify_url = site_url() . '/?pqp_paypal_ipn=process';
...
function wppp_render_paypal_button_form($args) {
    ...
    <?php echo $html_options; ?>
                    ...
                    if (!empty($other_amount)) {
                        ...
                        echo esc_attr($other_amount_label).' <input type="number" min="1" step="any" name="other_amount" value="" class="pqp_other_amt_input" style="max-width:80px;"> ' . esc_attr($currency);
                        echo '</div>';
                    }
